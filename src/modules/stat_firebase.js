	...
	if (docRef.data().latestMeetingId === null) {
		...
		querySnapshot.forEach(async (doc) => {
			...
			if (doc.data().meetingLimitUntil < now) {
				utils.statConsoleLog(`There is no valid meeting ongoing. Creating a new document...`);
				await createNewMeetingDocument();

			} else {
				meetingDocRef = firestore.doc(dbRootRef, 'meetings', doc.id);
				utils.statConsoleLog(`Joining an ongoing meeting (ID=${doc.id}).`);
			}
		})
	}
}
...
async function createNewMeetingDocument() {
	...
	try {
		...
		newDocId = docRef.id;
		meetingDocRef = firestore.doc(dbRootRef, 'meetings', newDocId);

	} catch (e) {
		console.error("Error adding document: ", e);

	} finally {
		utils.statConsoleLog(`New meeting document has set(${newDocId}).`);

		await firestore.updateDoc(dbRootRef, {
			lastTimeActive: firestore.Timestamp.now(),
			latestMeetingId: newDocId,
		});
